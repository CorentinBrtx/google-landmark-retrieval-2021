...
def transfer_images(data_folder: str, clean_csv: str, nb_landmarks: int = -1):
    ...
    images_paths = glob.glob(os.path.join(data_folder, "images_temp", "*/*/*/*.jpg"))
    for image in images_paths:
        ...
        if image_name not in images_clean or images_clean[image_name] > max_landmark:
            ...
            os.system(
                f"mkdir -p {os.path.dirname(get_path(os.path.join(data_folder, 'train'), image_name))}; "
                + f"mv {image} $_"
            )
...
def download_and_sort(
    ...
    os.makedirs(os.path.join(data_folder, "images_temp"), exist_ok=True)

    if clean_csv is None:
        clean_csv = os.path.join(data_folder, "train_clean.csv")
    ...
    os.system(
        f"bash ./src/data/google_download.sh train {begin} {end} {data_folder} {os.path.join(data_folder, 'images_temp')}"
    )

    logging.info("Transfering downloaded images")

    transfer_images(data_folder=data_folder, clean_csv=clean_csv, nb_landmarks=nb_landmarks)

    for i in range(begin, end + 1):
        os.system(f"rm {os.path.join(data_folder, f'images_{int_to_string(i)}.tar')}")
        os.system(f"rm {os.path.join(data_folder, f'md5.images_{int_to_string(i)}.txt')}")

    os.system(f"rm -r {os.path.join(data_folder,'images_temp')}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--data_folder",
        dest="data_folder",
        help="Folder to contain the data files.",
        required=True,
    )
    parser.add_argument(
        "-b",
    ...
    download_and_sort(args.data_folder, args.clean_csv, args.begin, args.end, args.nb_landmarks)
